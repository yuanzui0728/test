...
class Pin_Snailfishshop extends AdminController
{
	public function main()
	{
		...
		$goods_type = isset($_GPC['goods_type']) ? $_GPC['goods_type']:'pin';
		...
		$stock_notice_count = load_model_class('goods')->get_goods_count(" and grounding = 1 and total<= {$goods_stock_notice} and type = '{$goods_type}' ");//回收站商品数量
		//goods_stock_notice
		...
		if( !empty($type) && $type != 'all')
		{
			switch($type)
			{
				...
				break;
				case 'stock_notice':
					$condition .= " and grounding = 1 and total<= {$goods_stock_notice} ";
					break;
			}
		...
		include $this->display('pin/goodslist');
	}
