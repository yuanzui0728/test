...
class Statistics_Snailfishshop extends AdminController
{
	...
	public function index_data()
	{
		...
		$member_count = load_model_class('member')->get_member_count();
		...
		switch( $type )
		{
			case 'normal':
				...
				$today_member_count = load_model_class('member')->get_member_count(" and create_time > ".$today_time );
				//今日会员数量
				...
				if( $_W['role'] == 'agenter' )
				{
					...
					$order_ids_list = pdo_fetchall("select og.order_id,o.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
										" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and o.pay_time > {$today_time} and o.type <> 'integral' and o.order_status_id in (1,4,6,7,11,14) ", 
					...
					$today_pay_order_count = load_model_class('order')->get_order_count($today_pay_where);
					$result['today_pay_order_count'] = $today_pay_order_count;
					
					//get_order_sum($field=' sum(total) as total ' , $where = '',$uniacid = 0) 
					//total
					$today_pay_money_info = load_model_class('order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $today_pay_where);
				...
				if( $_W['role'] == 'agenter' )
				{
					...
					$goods_count = pdo_fetchcolumn('SELECT count(id) as count FROM ' . tablename('lionfish_comshop_good_common') . 
								' WHERE uniacid= '.$_W['uniacid'] . " and supply_id = ".$supper_info['id']);//lionfish_comshop_good_common
				}else{
					$total_tixian_money_all =  pdo_fetchcolumn('SELECT sum(money) as total_money FROM ' . tablename('lionfish_community_head_tixian_order') . 
				    ' WHERE uniacid= '.$_W['uniacid'] . "  and state = 0 ");
					$total_tixian_money_service_fare =  pdo_fetchcolumn('SELECT sum(service_charge) as total_service_charge FROM ' . tablename('lionfish_community_head_tixian_order') . 
				    ' WHERE uniacid= '.$_W['uniacid'] . "  and state = 0 ");
					...
					**/
					$total_commiss_money_all = pdo_fetchcolumn('SELECT sum(money) as total_money FROM ' . tablename('lionfish_community_head_commiss_order') . 
				    ' WHERE uniacid= '.$_W['uniacid'] . "  and (state = 1 or state =0) ");
					
					$result['total_commiss_money'] = sprintf("%.2f",$total_commiss_money_all);
					
					
					$all_pay_where = "  and order_status_id in (1,4,6,11,12,14) ";
					...
					$total_order_money = pdo_fetchcolumn('SELECT sum(total+shipping_fare-voucher_credit-fullreduction_money) as total FROM ' . 
									...
									' WHERE uniacid= '.$_W['uniacid'] . $all_pay_where );
				...
				if( $_W['role'] == 'agenter' )
				{
					...
					$goods_count = load_model_class('goods')->get_goods_count();
				}
				...
				$community_head_count =  pdo_fetchcolumn('SELECT count(id) as count FROM ' . tablename('lionfish_community_head') . 
				...
				if( $_W['role'] == 'agenter' )
				{
					...
					$order_ids_list = pdo_fetchall("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
										" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and o.order_status_id =3 ", 
										array(':uniacid' => $_W['uniacid']));
					$order_ids_arr = array();
					...
					$wait_pay_order_count = load_model_class('order')->get_order_count(" and order_status_id =3 {$total_where}");
				...
				if( $_W['role'] == 'agenter' )
				{
					...
					$order_ids_list = pdo_fetchall("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
										" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and o.order_status_id =1 ", 
										array(':uniacid' => $_W['uniacid']));
					$order_ids_arr = array();
					foreach($order_ids_list as $vv)
					{
					...
					$wait_order_count = load_model_class('order')->get_order_count(" and order_status_id =1 ");
				...
				if( $_W['role'] == 'agenter' )
				{
					...
					$order_ids_list = pdo_fetchall("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
										" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and o.order_status_id =12 ", 
										array(':uniacid' => $_W['uniacid']));
					$order_ids_arr = array();
					foreach($order_ids_list as $vv)
					{
					...
					$after_sale_order_count = load_model_class('order')->get_order_count(" and order_status_id = 12 ");
				...
				if( $_W['role'] == 'agenter' )
				{
					...
					$order_ids_list = pdo_fetchall("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
										" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and o.order_status_id =6 ", 
										array(':uniacid' => $_W['uniacid']));
					$order_ids_arr = array();
					foreach($order_ids_list as $vv)
					{
					...
					$wai_comment_order_count = load_model_class('order')->get_order_count(" and order_status_id = 6 ");
					$result['wai_comment_order_count'] = $wai_comment_order_count;
				}
				
				$result['wait_shen_order_comment_count'] = load_model_class('order')->get_wait_shen_order_comment();
				
				$result['stock_goods_count'] = load_model_class('goods')->get_goods_count('  and grounding =2 ');
				...
				if( $_W['role'] == 'agenter' )
				{
					...
					$order_ids_list = pdo_fetchall("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
										" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and o.date_added>={$begin_time} and o.date_added< {$end_time} ", 
										array(':uniacid' => $_W['uniacid']));
					$order_ids_arr = array();
					foreach($order_ids_list as $vv)
					{
					...
					$seven_order_count = load_model_class('order')->get_order_count(" and date_added>={$begin_time} and date_added< {$end_time} {$total_where} ");
				...
				if( $_W['role'] == 'agenter' )
				{
					...
					$order_ids_list = pdo_fetchall("select og.order_id,o.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
										" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and o.pay_time > {$begin_time} and o.order_status_id in (1,4,6,11,14) ", 
										array(':uniacid' => $_W['uniacid']));
					$order_ids_arr = array();
					...
					$seven_pay_money_info = load_model_class('order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $seven_pay_where);
				...
				if( $_W['role'] == 'agenter' )
				{
					...
					$order_ids_list = pdo_fetchall("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
										" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and o.pay_time > {$begin_time} and o.order_status_id = 7 ", 
										array(':uniacid' => $_W['uniacid']));
					$order_ids_arr = array();
					...
					$seven_refund_money_info = load_model_class('order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $seven_refund_pay_where);
				...
				//库存预警商品数量
				$goods_stock_notice_count = load_model_class('goods')->get_goods_count(" and total <= ".$goods_stock_notice." and grounding = 1  and type = 'normal' ");
				$result['goods_stock_notice_count'] = $goods_stock_notice_count;
				
				
				$apply_count = pdo_fetchcolumn("select count(id) from ".tablename('lionfish_community_head_tixian_order')." where uniacid=:uniacid and state=0",
							array(':uniacid' => $_W['uniacid']));
			...
			case 'pintuan':
				...
				$result['member_count'] = $member_count;
				//商品数量
				$goods_count = load_model_class('goods')->get_goods_count(" and type = 'pin' ");
				$result['goods_count'] = $goods_count;
				
				//订单数量
				$order_count = load_model_class('order')->get_order_count(" and type = 'pintuan' ");
				$result['order_count'] = $order_count;
				
				
				//待发货订单 1
				$wait_order_count = load_model_class('order')->get_order_count(" and order_status_id =1 and type = 'pintuan' ");
				$result['wait_order_count'] = $wait_order_count;
				
				//售后中订单 
				$after_sale_order_count = load_model_class('order')->get_order_count(" and order_status_id = 12 and type = 'pintuan' ");
				$result['after_sale_order_count'] = $after_sale_order_count;
				...
				//库存预警商品数量
				$goods_stock_notice_count = load_model_class('goods')->get_goods_count(" and total <= ".$goods_stock_notice." and type = 'pin' ");
				$result['goods_stock_notice_count'] = $goods_stock_notice_count;
	...
	function order_buy_data()
	{
		...
		$s_index = isset($_GPC['s_index']) ? $_GPC['s_index']:2;
		...
		switch( $type )
		{
			case 'normal':
				...
				if( $_W['role'] != 'agenter' )
				{
					...
					$sum_info = load_model_class('order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $where);
					...
					$order_ids_list = pdo_fetchall("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
										" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and  og.addtime >= {$begin_time} and og.addtime <={$end_time}  ", 
										array(':uniacid' => $_W['uniacid']));
	...
	public function load_echat_member_incr()
	{
		...
		for($i =7; $i>=1; $i--)
		{
			...
			$member_count = pdo_fetchcolumn('SELECT count(member_id) as count FROM ' . tablename('lionfish_comshop_member') . 
								' WHERE uniacid= '.$_W['uniacid'] . " and create_time >= ".$begin_time." and create_time < ".$end_time );
	...
	public function load_echat_head_incr()
	{
		...
		for($i =7; $i>=1; $i--)
		{
			...
			$member_count = pdo_fetchcolumn('SELECT count(member_id) as count FROM ' . tablename('lionfish_community_head') . 
								' WHERE uniacid= '.$_W['uniacid'] . " and state = 1 and addtime >= ".$begin_time." and addtime < ".$end_time );
	...
	public function load_echat_month_goods_sales()
	{
		...
		//lionfish_comshop_order  1 4 6 11 14  date_added total_quantity
		
		$sql = "SELECT og.goods_id,og.name ,sum( og.total + og.shipping_fare - og.voucher_credit - og.fullreduction_money ) as total,sum(og.quantity) as total_quantity     
				FROM ".tablename('lionfish_comshop_order_goods')." as og ,".tablename('lionfish_comshop_order')." as o 
				where og.order_id = o.order_id and o.uniacid=:uniacid and o.date_added >= {$begin_time} and o.date_added <={$end_time}  and o.order_status_id in(1,4,6,11,14) group by og.goods_id order by total desc limit 10 ";
	...
	public function load_echat_month_head_sales()
	{
		...
		//echo date( "Y-m-d", strtotime( "last day of next month" ) );  // PHP:  2009-04
		...
		$sql = " select sum(total+shipping_fare-voucher_credit-fullreduction_money) as total,head_id from ".tablename('lionfish_comshop_order').
				" where uniacid=:uniacid and date_added >= {$begin_time} and date_added <={$end_time} and order_status_id in(1,4,6,11,14) 
		...
		foreach( $list as $key => $val )
		{
			...
			$hd_info = pdo_fetch("select community_name,head_name from ".tablename('lionfish_community_head').
								" where uniacid=:uniacid and id=:id  ", array(':uniacid' => $_W['uniacid'], ':id' => $val['head_id'] ));
			...
			case 'normal':
				...
				for($i =7; $i>=1; $i--)
				{
					...
					if( $_W['role'] != 'agenter' )
					{
						...
						$sum_info = load_model_class('order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $where);
						$refund_sum_info = load_model_class('order')->get_order_sum(' sum(total+shipping_fare-voucher_credit-fullreduction_money) as total ' , $where_refund);
					
					}else{
						$supper_info = json_decode(base64_decode($_GPC['__lionfish_comshop_agent']), true);
						
						$order_ids_list = pdo_fetchall("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
											" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and  og.addtime >= {$begin_time} and og.addtime <={$end_time} and o.order_status_id in(1,4,6,11,14) ", 
											array(':uniacid' => $_W['uniacid']));
						
						$order_ids_list2 = pdo_fetchall("select og.order_id,og.total,og.shipping_fare,og.voucher_credit,og.fullreduction_money from ".tablename('lionfish_comshop_order_goods').
											" as og , ".tablename('lionfish_comshop_order')." as o  where  og.order_id =o.order_id and  og.uniacid=:uniacid and og.supply_id = ".$supper_info['id']." and  og.addtime >= {$begin_time} and og.addtime <={$end_time} and o.order_status_id in(7) ", 
											array(':uniacid' => $_W['uniacid']));
	...
	function load_goods_paihang()
	{
		...
		switch( $type )
		{
			case 'normal':
				...
				$list = load_model_class('order')->get_order_goods_group_paihang($where );
