...
class Configservice_Snailfishshop extends AdminController
{
	...
	public function index()
	{
		...
		if ($_W['ispost']) {
			check_permissions('config.configpay.index.edit');
			$data = ((is_array($_GPC['parameter']) ? $_GPC['parameter'] : array()));
			$data['config_service_user_number'] = trim($data['config_service_user_number']);
			$data['config_service_reply_content'] = trim($data['config_service_reply_content']);
			$data['config_service_first_reply_content'] = trim($data['config_service_first_reply_content']);
			
			load_model_class('config')->update($data);
			
			snialshoplog('configservice.edit', '修改系统设置-客服设置');
			show_json(1);
		}
		$data = load_model_class('config')->get_all_config();
		
		include $this->display('config/configservice/index');
	}
