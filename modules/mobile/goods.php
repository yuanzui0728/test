...
class Goods_Snailfishshop extends MobileController
{
	...
	**/
	...
	public function notify_order()
	{
		...
		$notify_order = pdo_fetch("select * from ".tablename('lionfish_comshop_notify_order')." where  uniacid=:uniacid order by state asc , id asc ", array(':uniacid' => $_W['uniacid']));
		...
		pdo_query("update ".tablename('lionfish_comshop_notify_order')." set state =state+1  where uniacid=:uniacid and id=:id ", array(':id' => $notify_order['id'],':uniacid' => $_W['uniacid']) );
