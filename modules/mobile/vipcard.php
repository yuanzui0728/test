...
class Vipcard_Snailfishshop extends MobileController
{
	...
	public function get_vipcard_baseinfo()
	{
		...
		$vipcard_unopen_headbg = load_model_class('front')->get_config_by_name('vipcard_unopen_headbg');
		$vipcard_effect_headbg = load_model_class('front')->get_config_by_name('vipcard_effect_headbg');
		$vipcard_afterefect_headbg = load_model_class('front')->get_config_by_name('vipcard_afterefect_headbg');
		...
		$vipcard_buy_pagenotice = load_model_class('front')->get_config_by_name('vipcard_buy_pagenotice');
		$vipcard_equity_notice = load_model_class('front')->get_config_by_name('vipcard_equity_notice');
		...
		$card_list = pdo_fetchall("select * from ".tablename('lionfish_comshop_member_card')." where uniacid=:uniacid ", 
					array(':uniacid' => $_W['uniacid']));
					
		$card_equity_list = pdo_fetchall("select * from ".tablename('lionfish_comshop_member_card_equity')." where uniacid=:uniacid ", 
							array(':uniacid' => $_W['uniacid']));
		...
		$is_hide_vipcard_vipgoods = load_model_class('front')->get_config_by_name('is_hide_vipcard_vipgoods');
		...
		$category_list = pdo_fetchall("select * from ".tablename('lionfish_comshop_goods_category')." where uniacid=:uniacid and cate_type='normal' and is_show=1 and pid=0 order by sort_order desc,id asc ", 
						array(':uniacid' => $_W['uniacid'] ));
	...
	public function get_vipgoods_list()
	{
		...
		if($gid > 0){
			...
			WHERE uniacid=:uniacid and is_show=1 and cate_type='normal' and pid = {$gid}  " . ' 
	...
	**/
	public function wxcharge()
	{
		...
		$card_info = pdo_fetch("select * from ".tablename('lionfish_comshop_member_card')." where uniacid=:uniacid and id=:id ", 
		...
		$member_info = pdo_fetch("select * from ".tablename('lionfish_comshop_member')." where uniacid=:uniacid and member_id=:member_id ", array(':member_id' => $member_id, ':uniacid' => $_W['uniacid']));
		...
		pdo_insert('lionfish_comshop_member_card_order', $member_charge_flow_data);
		...
		if($array['RETURN_CODE'] == 'SUCCESS' && $array['RESULT_CODE'] == 'SUCCESS'){
			...
			pdo_update('lionfish_comshop_member_card_order', array('formid' => $array['PREPAY_ID'] ), array('id' => $order_id));
