...
class WxPayApi
{
	...
	{
		global $_W;//global $_W; uniacid
		
		$wepro_appid = load_model_class('front')->get_config_by_name('wepro_appid', $_W['uniacid']);
		$wepro_partnerid = load_model_class('front')->get_config_by_name('wepro_partnerid', $_W['uniacid']);
	...
	**/
	...
	public static  function MakeSignfh($values)
	{
		...
		$pay_key = load_model_class('front')->get_config_by_name('wepro_key', $_W['uniacid']);
	...
	public static function payToUser($openid,$amount,$username,$desc,$partner_trade_no,$uniacid=0)
	{
		...
		$wepro_appid = load_model_class('front')->get_config_by_name('wepro_appid', $uniacid);
		
		
		$mchid = load_model_class('front')->get_config_by_name('wepro_partnerid', $uniacid);
		
		$wepro_key = load_model_class('front')->get_config_by_name('wepro_key', $uniacid);
		...
		$response = self::postXmlCurl($xml, $url, true ,6 , $uniacid);
	...
	public static function refund($inputObj, $timeOut = 6,$from_type='weixin',$uniacid = 0)
	{
		...
		$response = self::postXmlCurl($xml, $url, true, $timeOut, $uniacid);
	...
	{		
		...
		**/
		...
		if($useCert == true){
			$path = IA_ROOT.'/addons/lionfish_comshop/data/wxpaylogs/';
			
			$wechat_apiclient_cert_pem = load_model_class('front')->get_config_by_name('wechat_apiclient_cert_pem', $uniacid );
			$wechat_apiclient_key_pem = load_model_class('front')->get_config_by_name('wechat_apiclient_key_pem', $uniacid );
