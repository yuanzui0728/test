...
public function getSnsapiUserinfo($auth_accsss_info)
{
 ...
 public  function getAutoAccessToken($code)
 {
  ...
  private function getJsApiTicket() {
	...
	$data = cache_load($_W['uniacid'].'jsapi_ticket');
    ...
    if ($data->expire_time < time() || empty( $data )) {
      ...
      if ($ticket) {
		...
		cache_write($_W['uniacid'].'jsapi_ticket', $data);
      }
    } else {
...
**/	
  public function getWeQrcode($quan_sn)
  {
	  ...
	  {
	      $access_token = $this->getweAccessToken(0,true);
	      $url = "https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token={$access_token}";
  ...
  public function getweAccessToken($uniacid=0,$is_parse = false) {
	...
	$data = cache_load($uniacid.'wepro_weaccess_token');
	
	if (empty( $data ) || $data['expire_time'] < time() || $is_parse ) {
      ...
      if ($access_token) {
		...
		cache_write($uniacid.'wepro_weaccess_token', $data);
  ...
  public function reflush_token($uniacid = 0)
  {
		global $_W;
		...
		$data = cache_load($uniacid.'access_token');
		...
		//  var_dump($data ,$res,$rr);die();
		  if ($access_token) {
			...
			cache_write($uniacid.'access_token', $data);
  ...
  public function getAccessToken() {
    ...
    return $this->getweAccessToken(0,false);
  }
