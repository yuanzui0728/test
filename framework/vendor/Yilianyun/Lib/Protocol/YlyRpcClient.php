...
class YlyRpcClient{
    ...
    public function call($action, array $params)
    {
        ...
        if (
		   ...
		   show_json(0, '易联云错误：Call method ' . $action . ' error code is ' . $response->error . ' error message is ' . $errorDescription);
    ...
    public function send($data,$url)
    {
        ...
        if (curl_errno($curl)) {
            if ($log != null) {
                $log->error("易联云错误：error: " . curl_error($curl));
            }
			show_json(0, "易联云错误：error: " . curl_error($curl) );	
