<?php
class Cache_SnailFishShopModel
{
		...
		if (function_exists('redis')) {
			...
			if (!is_error($redis)) {
				if (stripos($uniacid, 'global') !== false) {
					return 'lionfish_comshop_syscache_' . $_W['setting']['site']['key'] . '_global_' . $key;
				}
				...
				return 'lionfish_comshop_syscache_' . $_W['setting']['site']['key'] . '_' . $uniacid . '_' . $account_key . '_' . $key;
			}
		}
	...
	public function get($key = '', $uniacid = '')
	{
		...
		if (function_exists('redis')) {
			...
			if (!is_error($redis)) {
				$prefix = '__lionfish_comshop__format__::';
