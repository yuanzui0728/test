...
class Common_SnailFishShopModel
{
	public $public_build;
	...
	public function updateSysset($values, $uniacid = 0)
	{
		...
		$setdata = pdo_fetch('select id, sets from ' . tablename('lionfish_comshop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));

		if (empty($setdata)) {
			pdo_insert('lionfish_comshop_sysset', array('sets' => iserializer($values), 'uniacid' => $uniacid));
		}
		 else {
			...
			pdo_update('lionfish_comshop_sysset', array('sets' => iserializer($sets)), array('id' => $setdata['id']));
		}

		$setdata = pdo_fetch('select * from ' . tablename('lionfish_comshop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
	...
	public function updatePluginset($values, $uniacid = 0)
	{
		...
		$setdata = pdo_fetch('select id, plugins from ' . tablename('lionfish_comshop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));

		if (empty($setdata)) {
			pdo_insert('lionfish_comshop_sysset', array('plugins' => iserializer($values), 'uniacid' => $uniacid));
		}
		 else {
			...
			pdo_update('lionfish_comshop_sysset', array('plugins' => iserializer($plugins)), array('id' => $setdata['id']));
		}
	...
	public function getSec($uniacid = 0)
	{
		...
		$set = pdo_fetch('select sec from ' . tablename('lionfish_comshop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
	...
	{
		return pdo_fetchall('SELECT  a.uniacid,a.name FROM ' . tablename('lionfish_comshop_sysset') . ' s  ' . ' left join ' . tablename('account_wechats') . ' a on a.uniacid = s.uniacid' . ' left join ' . tablename('account') . ' acc on acc.uniacid = a.uniacid where acc.isdeleted=0 GROUP BY uniacid');
	}
	...
	public function getCopyright($ismanage = false)
	{
		...
		if (!(is_array($copyrights))) {
			$copyrights = pdo_fetchall('select *  from ' . tablename('lionfish_comshop_system_copyright'));
