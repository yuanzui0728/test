...
class Caiji_SnailFishShopModel
{
	...
	public function get_item_taobao($itemid = '', $taobaourl = '', $cates, $merchid = 0) 
	{
		...
		$mockData = htmlspecialchars_decode($data['mockData']);
		$mock_js_arr = json_decode($mockData, true);
		
		$has_option = false;
		if($mock_js_arr['feature']['hasSku'])
		{
			//有规格
			$has_option = true;
		}
		
		$goods_price = $mock_js_arr['price']['price']['priceText'];
		...
		//http://hws.m.taobao.com/cache/wdesc/5.0/?id=
		...
		$goods_data = array();
		$goods_data['uniacid'] = $_W['uniacid'];
		$goods_data['goodsname'] = $item['title'];
		$goods_data['subtitle'] = '';
		$goods_data['grounding'] = 2;
		$goods_data['price'] = $goods_price;
		$goods_data['productprice'] = $goods_price;
		$goods_data['sales'] = 0;
		$goods_data['type'] = 'normal';
		$goods_data['showsales'] = 1;
		$goods_data['dispatchtype'] = 1;
		$goods_data['dispatchid'] = 0;
		$goods_data['dispatchprice'] = 0;
		$goods_data['codes'] = '';
		$goods_data['weight'] = 0;
		$goods_data['total'] = 100;
		$goods_data['hasoption'] = $has_option ? 1 : 0;
		$goods_data['is_index_show'] = 0;
		$goods_data['credit'] = 0;
		$goods_data['buyagain'] = 0;
		$goods_data['buyagain_condition'] = 0;
		$goods_data['buyagain_sale'] = 0;
		$goods_data['addtime'] = time();
		
		
		pdo_insert('lionfish_comshop_goods', $goods_data);
		...
		if (0 < $piclen) 
		{
			...
			foreach($thumb_url as $img_val)
			{
				$img_data = array();
				$img_data['uniacid'] = $_W['uniacid'];
				$img_data['goods_id'] = $goods_id;
				$img_data['image'] = save_media($img_val);
				$img_data['thumb'] = save_media( file_image_thumb_resize($img_val,100));
				pdo_insert('lionfish_comshop_goods_images', $img_data);
			}
		}
		...
		if($mock_js_arr['feature']['hasSku'])
		{
			...
			$mult_option_item = $mock_js_arr['skuCore']['sku2info'];
			...
			foreach($props as $val)
			{
				...
				$goods_option['title'] = $title;
				$goods_option['displayorder'] = 1;
				
				pdo_insert('lionfish_comshop_goods_option', $goods_option);
				$option_id = pdo_insertid();
				
				$pid_optionid[$pid] = $option_id;
				...
				foreach($val['values'] as $vv)
				{
					...
					$goods_option_item_data['title'] = $vv['name'];
					$goods_option_item_data['thumb'] = $this->save_image($vv['image'], false);
					$goods_option_item_data['displayorder'] = 1;
					
					
					pdo_insert('lionfish_comshop_goods_option_item', $goods_option_item_data);
					$option_item_id = pdo_insertid();
					$vid_itemid[$vv['vid']] = $option_item_id;
					
				}
			}
			
			
			foreach($skus as $new_key => $val)
			{
				$sku_id = $new_key;
				
				$propPath = $val['propPath'];
				
				$prop_arr = explode(';', $propPath);
				
				$option_item_ids_arr = array();
				
				
				$item_mult_info = $mult_option_item[$sku_id];
				
				$price = $item_mult_info['price']['priceText'];
				$mult_quantity = $item_mult_info['quantity'];
				
				foreach($prop_arr as $vv)
				{
					$pid_vid = explode(':', $vv);
					$pid = $pid_vid[0];
					$vid = $pid_vid[1];
					$option_item_ids_arr[] = $vid_itemid[$vid];
					//商品id,规格值关联id,商品价格，商品原价，拼团价格，库存，商品成本价，商品编码，重量，标题
				}
				
				
				asort($option_item_ids_arr);
				$option_item_ids = implode('_', $option_item_ids_arr);
				$option_item_title_arr = array();
				$option_item_title_str = '';
				if(!empty($option_item_ids_arr))
				{
					foreach($option_item_ids_arr as $item_id )
					{
						//lionfish_comshop_goods_option_item
						$goods_option_data = pdo_fetch('select * from ' . tablename('lionfish_comshop_goods_option_item') . ' 
						where uniacid=:uniacid  and id=:id limit 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $item_id));
						$option_item_title_arr[] = $goods_option_data['title'];
					}
				}
				
				$option_item_title_str = implode('_', $option_item_title_arr);
				...
				$snailfish_goods_option_item_value_data['productprice'] =  $price;
				$snailfish_goods_option_item_value_data['pinprice'] =  $price; 
				
				$snailfish_goods_option_item_value_data['marketprice'] =  $price;
				$snailfish_goods_option_item_value_data['stock'] =  $mult_quantity;
				$snailfish_goods_option_item_value_data['costprice'] =  $price;
				$snailfish_goods_option_item_value_data['goodssn'] =  '';
				$snailfish_goods_option_item_value_data['weight'] =  '';
				$snailfish_goods_option_item_value_data['title'] =  $option_item_title_str;
		...
		$html = $this->html_to_images($html);
		
		//lionfish_comshop_goods_to_category
		if (!empty($cates)) 
		{
			$item['cates'] = implode(',', $cates);
			foreach($cates as $cate_id)
			{
				...
				$cate_data['cate_id'] = $cate_id;
				$cate_data['goods_id'] = $goods_id;
				pdo_insert('lionfish_comshop_goods_to_category', $cate_data);
			}
		}
		
		$goods_common_data = array();
		$goods_common_data['uniacid'] = $_W['uniacid'];
		$goods_common_data['goods_id'] = $goods_id;
		$goods_common_data['quality'] = '';
		$goods_common_data['seven'] = '';
		$goods_common_data['repair'] = '';
		$goods_common_data['labelname'] =serialize(array());
		$goods_common_data['share_title'] = '';
		$goods_common_data['share_description'] = '';
		$goods_common_data['content'] = $html;
		pdo_insert('lionfish_comshop_good_common', $goods_common_data);
		
		return array('result' => '1', 'goodsid' => $goods_id);
	}
	
	private function html_to_images($detail = '')
	{
	...
	public function save_taobao_goods($item = array(), $catch_url = '') 
	{
		...
		var_dump($data);
		die();
		$goods = pdo_fetch('select * from ' . tablename('ewei_shop_goods') . ' where  catch_id=:catch_id and catch_source=\'taobao\' and uniacid=:uniacid and merchid=:merchid', 
	...
	{
		$url = "https://acs.m.taobao.com/h5/mtop.taobao.detail.getdetail/6.0/?data=%7B%22itemNumId%22%3A%22{$itemid}%22%7D&qq-pf-to=pcqq.group";
